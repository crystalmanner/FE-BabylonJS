...
class Lane {
    ...
    drawSegs() {
        var mat_road_no_splitter = new BABYLON.StandardMaterial("mat_road_no_splitter", this.scene);
        mat_road_no_splitter.diffuseTexture = new BABYLON.Texture("assets/images/material-road-no-splitter.jpg", this.scene);

        var mat_road_with_splitter = new BABYLON.StandardMaterial("mat_road_with_splitter", this.scene);
        mat_road_with_splitter.diffuseTexture = new BABYLON.Texture("assets/images/material-road-with-splitter.jpg", this.scene);
        ...
        for (var i = 0; i < this.paint.length; i++) {
            ...
            for (var j = 0; j < pts.length - 1; j++) {
                ...
                {
                    // add 3d material
                    var plane_road_box = BABYLON.MeshBuilder.CreatePlane("plane_road_depth", {height: LANE_WIDE, width: LANE_WIDE}, this.scene);
                    plane_road_box.position = new BABYLON.Vector3(pts[j][0]/SCALE_METER2CM, pts[j][1]/SCALE_METER2CM, pts[j][2]/SCALE_METER2CM);
                    plane_road_box.material = mat_road_no_splitter;
                ...
                else 
                {
                    ...
                    plane_road.material = mat_road_with_splitter;
                ...
                if(pts[j][1] > 1590 && pts[j][1] < 1595 || pts[j][1] > 1470 && pts[j][1] < 1472 || 
                    pts[j][1] > 1000 && pts[j][1] < 1005 || pts[j][1] > 1900 && pts[j][1] < 1905)
                {
                    var cone1 = BABYLON.MeshBuilder.CreateCylinder("cone", {diameterBottom: 1.5, height: pts[j][1]/SCALE_METER2CM + LANE_DEPTH, tessellation: 96}, this.scene);
