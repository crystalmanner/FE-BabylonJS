...
function loadCarFinished() {
    g_carIndex --;
    if(g_carIndex == 0)
    {
        // alert('Mesh loading finished');
    }
}

function loadCarMeshes(scene) {
    g_carIndex = VEHICLE_TYPES.length * COLORS.length - 1;
    for (var index = 0; index < VEHICLE_TYPES.length * COLORS.length; index++)
    {
        BABYLON.SceneLoader.ImportMesh("", "assets/", "car.babylon", scene, function (newMeshes) {
            g_carMeshes[g_carIndex] = newMeshes[0];
            g_carMeshes[g_carIndex].scaling = new BABYLON.Vector3(CAR_SCALE, CAR_SCALE, CAR_SCALE);
            g_carMeshes[g_carIndex].position = new BABYLON.Vector3(0, -100, g_carIndex * 20);
            // g_carMeshes[g_carIndex].rotation.z = -Math.PI/7;
            loadCarFinished();
        });
    }
}
...
function createParentVehicles(scene) {
    ...
    for (var i = 0; i < VEHICLE_TYPES.length; i++) { // VEHICLE_CAR, VEHICLE_BUS, VEHICLE_TRUCK
        for (var j = 0; j < COLORS.length; j++) { // COLOR_BLUE, COLOR_YELLOW, COLOR_WHITE, COLOR_BLACK, COLOR_GREEN, COLOR_BKAIR
            ...
            // var box = g_carMeshes[g_pVehicles_len];
            // box.position.y = -100;
            // vehicle.box = box;
            // g_pVehicles[g_pVehicles_len ++] = vehicle;
        }
...
function animation(tick32) {
    ...
    for (var i = 0; i < frame.length; i++) {
        ...
        if ( freeMeshes.length == 0) {
            ...
            // g_cVehicles[g_cVehicles_len].instance.scaling = new BABYLON.Vector3(CAR_SCALE, CAR_SCALE, CAR_SCALE);
            // console.log(g_cVehicles[g_cVehicles_len].instance.position);
            g_cVehicles_len ++;
        }
        else {
            ...
            // g_cVehicles[freeMeshes[0].idx].instance.scaling = new BABYLON.Vector3(CAR_SCALE, CAR_SCALE, CAR_SCALE);
            // console.log(g_cVehicles[freeMeshes[0].idx].instance.position);
        }
    }
}
...
function init() {
    for (var i = 0; i < g_cVehicles_len; i++) {
        ...
        // g_cVehicles[i].instance.rotation = new BABYLON.Vector3(-Math.PI/2, 0, 0);
